...
RGBTRIPLE calculate_weighted_sum(int index_h, int index_w, int height, int width, RGBTRIPLE image[height][width])
{
    ...
    weighted_sum.rgbtBlue = (int)round( sqrt( pow((float)xgreen, 2.0F) + pow((float)ygreen, 2.0F) ));
    weighted_sum.rgbtBlue = (int)round( sqrt( pow((float)xred,   2.0F) + pow((float)yred,   2.0F) ));
